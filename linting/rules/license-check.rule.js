...
const EARLIEST_VALID_YEAR = 2025;

const OLD_COPYRIGHT_HEADER =
	'Copyright (c) Mysten Labs, Inc.';
const LICENSE_IDENTIFIER =
	'SPDX-License-Identifier: Apache-2.0';

const MISSING_HEADER_MESSAGE =
	'Missing or incorrect license header.';
const MISSING_MODIFICATION_MESSAGE =
	'Add modification notice to the license header.';
...
function createCopyrightYearPattern() {
	...
	return {
		header: new RegExp(
			`Copyright \\(c\\) (${years.join('|')}) IOTA Stiftung`,
		),
...
const {
	header: copyrightPattern,
	modificationHeader: modificationCopyrightHeader,
...
function checkHeader(node, context) {
	...
	const hasValidIotaCopyrightHeader =
		firstComment && copyrightPattern.test(firstComment);
	const hasOldCopyrightHeader = firstComment?.includes(
	...
	if (
		firstComment &&
		!hasValidIotaCopyrightHeader &&
		!hasOldCopyrightHeader
	) {
	...
	if (
		...
		const hasModificationHeader =
			modificationComment &&
			modificationCopyrightHeader.test(modificationComment);
